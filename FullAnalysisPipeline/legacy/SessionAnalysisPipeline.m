...
for rec = 1:size(MIDTable,1) 
    ...
    [AI(rec,:,:), CI(rec,:), R(rec), Monocularity(rec,:), Eye(rec,:), Eye_AI(rec,:), delta_bias(rec,:), Neuro(rec), LFP_Data(rec), BehaviorData(rec)] =...
        Stimulation_ClusteringIndexPipeline(MIDTable.Monkey(rec,:), MIDTable.Paths(rec,:), MIDTable.Names(rec,:), MIDTable.QuickNames(rec,:), MIDTable.Names_2D(rec,:), MIDTable.StimElec(rec), makePlots);
