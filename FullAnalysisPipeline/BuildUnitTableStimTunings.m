...
%   ExcludeFROutliers     - apply trial-level modified-Z/MAD filtering
%                           separately by Stim status/cue/coherence/channel
%                           (true)
%   FROutlierThreshold    - absolute modified-Z cutoff (3.5)
%   FROutlierMinimumTrials - minimum finite trials before testing (8)
...
arguments
    ...
    options.ExcludeFROutliers (1, 1) logical = true
    options.FROutlierThreshold (1, 1) double ...
        {mustBeFinite, mustBePositive} = 3.5
    options.FROutlierMinimumTrials (1, 1) double ...
...
schemaVersion = "unit_table_stim_v2_mad_outliers";
...
PipelineMetadata = struct( ...
    ...
    'ExcludeFROutliers', options.ExcludeFROutliers, ...
    'FROutlierThreshold', options.FROutlierThreshold, ...
    'FROutlierMinimumTrials', options.FROutlierMinimumTrials, ...
...
for row = rows
    ...
    reusableCompletedRow = isReusableRow( ...
        unit_table_stim, row, sourceSignature, schemaVersion);
    ...
    [unit_table_stim, recovered] = recoverFromTrialFR( ...
        ...
        makeFROutlierSettings(options), schemaVersion);
    if recovered
        unit_table_stim.stim_tuning_duration_seconds(row) = toc(rowStart);
    ...
    duplicateRow = findReusableSourceRow( ...
        unit_table_stim, row, sourceKey, sourceSignature, schemaVersion);
...
tableOut.stim_tuning_FR_outlier_enabled = false(n, 1);
tableOut.stim_tuning_FR_outlier_threshold = nan(n, 1);
tableOut.stim_tuning_FR_outlier_minimum_trials = nan(n, 1);
tableOut.stim_tuning_FR_outlier_observation_count = nan(n, 1);
tableOut.stim_tuning_FR_outlier_trial_count = nan(n, 1);
...
signature = schemaVersion + "|" + extractorSignature + ...
    ...
    "|ExcludeFROutliers=" + string(options.ExcludeFROutliers) + ...
    "|FROutlierThreshold=" + string(options.FROutlierThreshold) + ...
    "|FROutlierMinimumTrials=" + ...
    string(options.FROutlierMinimumTrials) + ...
...
function settings = makeFROutlierSettings(options)
settings = struct( ...
    'Enabled', options.ExcludeFROutliers, ...
    'Threshold', options.FROutlierThreshold, ...
    'MinimumTrials', options.FROutlierMinimumTrials);
end
...
function tf = isReusableRow( ...
    tableData, row, sourceSignature, schemaVersion)
success = startsWith(tableData.stim_tuning_status(row), "Success");
sameSchema = tableData.stim_tuning_schema_version(row) == schemaVersion;
...
if success && sameSchema && sameSource && trialFileExists && curvesPresent
    try
        loaded = load(trialFile, 'StimTrialFR');
        trialFileValid = isfield(loaded, 'StimTrialFR') && ...
            isfield(loaded.StimTrialFR, 'SchemaVersion') && ...
            string(loaded.StimTrialFR.SchemaVersion) == schemaVersion && ...
            ...
            isfield(loaded.StimTrialFR, 'FiringRateOutlierMask') && ...
            size(loaded.StimTrialFR.FiringRateHz, 1) == ...
            height(loaded.StimTrialFR.TrialSummary) && ...
            isequal(size(loaded.StimTrialFR.FiringRateOutlierMask), ...
            size(loaded.StimTrialFR.FiringRateHz));
    catch
        trialFileValid = false;
    end
end
tf = success && sameSchema && sameSource && trialFileExists && curvesPresent && ...
    trialFileValid;
end


function row = findReusableSourceRow(tableData, currentRow, ...
    sourceKey, sourceSignature, schemaVersion)
row = [];
if currentRow <= 1
    return
end
...
for candidate = candidates(:)'
    if isReusableRow( ...
            tableData, candidate, sourceSignature, schemaVersion)
        row = candidate;
        return
    end
end
...
valueColumns = ["stim_tuning_num_units", "stim_tuning_status", ...
    ...
    "stim_tuning_FR_outlier_enabled", ...
    "stim_tuning_FR_outlier_threshold", ...
    "stim_tuning_FR_outlier_minimum_trials", ...
    "stim_tuning_FR_outlier_observation_count", ...
    "stim_tuning_FR_outlier_trial_count", ...
...
tableData.stim_tuning_FR_outlier_enabled(row) = false;
end


function [tableData, recovered] = recoverFromTrialFR( ...
    tableData, row, trialFile, sourceSignature, ...
    applyEyeCheck, requireEyeCheck, expectedOutlierSettings, schemaVersion)
recovered = false;
if ~isfile(trialFile)
    return
end
try
    ...
    required = {'SchemaVersion', 'SourceSignature', 'FiringRateHz', ...
        'FiringRateOutlierMask', 'OutlierSettings', 'TrialSummary', ...
        'Coherence', 'ConditionNames', 'ChannelMap', 'ExtractionSummary'};
    if ~all(isfield(trial, required)) || ...
            string(trial.SchemaVersion) ~= schemaVersion || ...
            string(trial.SourceSignature) ~= sourceSignature || ...
            size(trial.FiringRateHz, 1) ~= height(trial.TrialSummary) || ...
            ~isequal(size(trial.FiringRateOutlierMask), ...
            size(trial.FiringRateHz)) || ...
            ~outlierSettingsMatch( ...
            trial.OutlierSettings, expectedOutlierSettings)
        return
    end
    ...
    [noStim, stim, merged] = summarizeTrialFiringRates( ...
        trial.FiringRateHz, trial.FiringRateOutlierMask, ...
        trial.TrialSummary, trial.Coherence);
...
function tf = outlierSettingsMatch(actual, expected)
required = {'Enabled', 'Threshold', 'MinimumTrials'};
tf = isstruct(actual) && all(isfield(actual, required)) && ...
    logical(actual.Enabled) == logical(expected.Enabled) && ...
    double(actual.Threshold) == double(expected.Threshold) && ...
    double(actual.MinimumTrials) == double(expected.MinimumTrials);
end


function [noStim, stim, merged] = summarizeTrialFiringRates( ...
    firingRate, outlierMask, trialSummary, coherence)
noStim = summarizeTrialGroup( ...
    firingRate, outlierMask, trialSummary, coherence, false);
stim = summarizeTrialGroup( ...
    firingRate, outlierMask, trialSummary, coherence, true);
merged = summarizeTrialGroup( ...
    firingRate, outlierMask, trialSummary, coherence, []);
...
function result = summarizeTrialGroup( ...
    firingRate, outlierMask, trialSummary, coherence, electricalStimValue)
cueCount = 4;
...
count = zeros(cueCount, coherenceCount, channelCount);
for cue = 1:cueCount
    for coherenceIndex = 1:coherenceCount
        ...
        nominalCount = nnz(mask);
        if nominalCount == 0
            continue
        end
        values = reshape(firingRate(mask, :, 1), [], channelCount);
        flagged = reshape(outlierMask(mask, :, 1), [], channelCount);
        values(flagged) = NaN;
        effectiveCount = sum(isfinite(values), 1);
        count(cue, coherenceIndex, :) = effectiveCount;
        meanFR(cue, coherenceIndex, :) = mean(values, 1, 'omitnan');
        semFR(cue, coherenceIndex, :) = ...
            std(values, 0, 1, 'omitnan') ./ sqrt(effectiveCount);
    end
end
...
function [Neuro, TrialSummary, summary] = retainExpectedChannels( ...
    Neuro, TrialSummary, summary, tableData, row)
...
if numel(retainedIndices) < originalCount
    ...
    Neuro.FiringRateOutlierMask = ...
        Neuro.FiringRateOutlierMask(:, retainedIndices, :);
...
settings = Neuro.FROutlierAudit.Settings;
[recomputedOutlierMask, retainedOutlierAudit] = ...
    DetectStimFiringRateOutliers( ...
    Neuro.TrialFiringRate, TrialSummary, ...
    Enabled=logical(settings.Enabled), ...
    Threshold=double(settings.Threshold), ...
    MinimumTrials=double(settings.MinimumTrials));
if ~isequal(recomputedOutlierMask, Neuro.FiringRateOutlierMask)
    error('UnitTableStim:RetainedOutlierMaskMismatch', ...
        ['Recomputed outlier mask changed after selecting the expected ' ...
        'acquisition channels.']);
end
Neuro.FROutlierAudit = retainedOutlierAudit;
trialOutlierCount = reshape(sum( ...
    Neuro.FiringRateOutlierMask, [2 3]), [], 1);
TrialSummary.HasFROutlier = trialOutlierCount > 0;
TrialSummary.FROutlierObservationCount = trialOutlierCount;
Neuro.FROutlierAudit.OutlierObservationCount = ...
    nnz(Neuro.FiringRateOutlierMask);
Neuro.FROutlierAudit.TrialWithAnyOutlierCount = ...
    nnz(trialOutlierCount > 0);
Neuro.FROutlierAudit.TrialOutlierObservationCount = trialOutlierCount;
summary.FROutlierFilter = Neuro.FROutlierAudit;
summary.FROutlierObservationCount = ...
    Neuro.FROutlierAudit.OutlierObservationCount;
summary.FROutlierTrialCount = ...
    Neuro.FROutlierAudit.TrialWithAnyOutlierCount;
end
...
if isfield(Neuro, 'Trials') && ...
        isfield(Neuro.Trials, 'ValidNumTrials')
    Neuro.Trials.ValidNumTrials = ...
        Neuro.Trials.ValidNumTrials(:, :, retainedIndices, :);
    Neuro.Trials.OutlierCount = ...
        Neuro.Trials.OutlierCount(:, :, retainedIndices, :);
end
end
...
if isfield(tuning, 'Trials') && ...
        isfield(tuning.Trials, 'ValidNumTrials')
    tuning.Trials.ValidNumTrials = ...
        tuning.Trials.ValidNumTrials(:, :, retainedIndices, :);
    tuning.Trials.OutlierCount = ...
        tuning.Trials.OutlierCount(:, :, retainedIndices, :);
end
end
...
pooledValidCounts = Neuro.WithZero.Pooled.Trials.ValidNumTrials;
separateValidCounts = ...
    Neuro.WithZero.NoStim.Trials.ValidNumTrials + ...
    Neuro.WithZero.Stim.Trials.ValidNumTrials;
if ~isequal(pooledValidCounts, separateValidCounts)
    error('UnitTableStim:PooledValidCountMismatch', ...
        ['Merged effective counts after outlier exclusion do not equal ' ...
        'NoStim plus Stim effective counts.']);
end
...
noStimN = Neuro.WithZero.NoStim.Trials.ValidNumTrials;
stimN = Neuro.WithZero.Stim.Trials.ValidNumTrials;
pooledN = pooledValidCounts;
...
if ~isfield(Neuro, 'FiringRateOutlierMask') || ...
        ~isequal(size(Neuro.FiringRateOutlierMask), size(trialFR))
    error('UnitTableStim:OutlierMaskSizeMismatch', ...
        'Firing-rate outlier mask must match the trial-FR array size.');
end
if any(Neuro.FiringRateOutlierMask & ~isfinite(trialFR), 'all')
    error('UnitTableStim:OutlierMaskOnMissingFR', ...
        'The outlier mask flags a nonfinite firing-rate observation.');
end
end
...
tableData.stim_tuning_n_noStim{row} = ...
    unitOne(noStim.Trials.ValidNumTrials);
tableData.stim_tuning_n_stim{row} = ...
    unitOne(stim.Trials.ValidNumTrials);
tableData.stim_tuning_n_merged{row} = ...
    unitOne(merged.Trials.ValidNumTrials);
...
if isfield(summary, 'FROutlierFilter')
    settings = summary.FROutlierFilter.Settings;
    tableData.stim_tuning_FR_outlier_enabled(row) = settings.Enabled;
    tableData.stim_tuning_FR_outlier_threshold(row) = settings.Threshold;
    tableData.stim_tuning_FR_outlier_minimum_trials(row) = ...
        settings.MinimumTrials;
    tableData.stim_tuning_FR_outlier_observation_count(row) = ...
        summary.FROutlierFilter.OutlierObservationCount;
    tableData.stim_tuning_FR_outlier_trial_count(row) = ...
        summary.FROutlierFilter.TrialWithAnyOutlierCount;
end
if isfield(summary, 'EyeCheck')
    tableData.stim_tuning_eye_check_status(row) = ...
...
StimTrialFR.FiringRateOutlierMask = Neuro.FiringRateOutlierMask;
StimTrialFR.OutlierSettings = Neuro.FROutlierAudit.Settings;
StimTrialFR.OutlierAudit = Neuro.FROutlierAudit;
...
frOutlierEnabled = tableData.stim_tuning_FR_outlier_enabled;
frOutlierThreshold = tableData.stim_tuning_FR_outlier_threshold;
frOutlierMinimumTrials = ...
    tableData.stim_tuning_FR_outlier_minimum_trials;
frOutlierObservations = ...
    tableData.stim_tuning_FR_outlier_observation_count;
frOutlierTrials = tableData.stim_tuning_FR_outlier_trial_count;
...
manifest = table(row, monkey, date, stimElec, recordingFolder, status, ...
    ...
    noStimTrials, stimTrials, excludedTrials, frOutlierEnabled, ...
    frOutlierThreshold, frOutlierMinimumTrials, ...
    frOutlierObservations, frOutlierTrials, eyeCheckStatus, numUnits, ...
    ...
    'FROutlierEnabled', 'FROutlierThreshold', ...
    'FROutlierMinimumTrials', 'FROutlierObservations', ...
    'FROutlierTrials', ...
