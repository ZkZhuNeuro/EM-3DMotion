...
totalFROutlierObservations = 0;
totalFROutlierTrials = 0;
...
for row = 1:rowCount
    ...
    requiredFields = {'SourceSignature', 'FiringRateHz', ...
    ...
    if ~isequal(size(trial.FiringRateOutlierMask), ...
            size(trial.FiringRateHz))
        error('UnitTableStimValidation:OutlierMaskSizeMismatch', ...
            'Row %d outlier mask does not match its firing-rate array.', row);
    end
    [expectedOutlierMask, ~] = DetectStimFiringRateOutliers( ...
        trial.FiringRateHz, trial.TrialSummary, ...
        Enabled=logical(trial.OutlierSettings.Enabled), ...
        Threshold=double(trial.OutlierSettings.Threshold), ...
        MinimumTrials=double(trial.OutlierSettings.MinimumTrials));
    if ~isequal(expectedOutlierMask, trial.FiringRateOutlierMask)
        error('UnitTableStimValidation:OutlierMaskMismatch', ...
            'Row %d saved outlier mask differs from recomputation.', row);
    end
    ...
    nominalNoStim = nnz(trial.TrialSummary.Included & ...
        ~trial.TrialSummary.ElectricalStim);
    nominalStim = nnz(trial.TrialSummary.Included & ...
        trial.TrialSummary.ElectricalStim);
    if nominalNoStim ~= tableData.stim_tuning_noStim_trial_count(row) || ...
            nominalStim ~= tableData.stim_tuning_stim_trial_count(row)
        error('UnitTableStimValidation:TrialCountMismatch', ...
            'Row %d scalar and cellwise trial counts differ.', row);
    end
    outlierObservationCount = nnz(trial.FiringRateOutlierMask);
    outlierTrialCount = nnz(any( ...
        trial.FiringRateOutlierMask, [2 3]));
    if outlierObservationCount ~= ...
            tableData.stim_tuning_FR_outlier_observation_count(row) || ...
            outlierTrialCount ~= ...
            tableData.stim_tuning_FR_outlier_trial_count(row)
        error('UnitTableStimValidation:OutlierCountMismatch', ...
            'Row %d outlier counts differ from its trial-FR file.', row);
    end
    totalFROutlierObservations = totalFROutlierObservations + ...
        outlierObservationCount;
    totalFROutlierTrials = totalFROutlierTrials + outlierTrialCount;
...
ValidationReport = table( ...
    ...
    "FROutlierObservations"; "FROutlierTrials"; ...
    ...
    sum(tableData.stim_tuning_stim_trial_count); ...
    totalFROutlierObservations; totalFROutlierTrials; channelTrimCount; ...
...
count = zeros(4, numel(coherence), channelCount);
for cue = 1:4
