...
%   ExcludeFROutliers  - exclude channel/unit trial observations using a
%                        modified Z-score based on median and MAD (true)
%   FROutlierThreshold - absolute modified-Z threshold (3.5)
%   FROutlierMinimumTrials - minimum finite trials in a Stim status x cue x
%                        coherence x channel x unit cell before testing (8)
...
[frOutlierMask, frOutlierAudit] = DetectStimFiringRateOutliers( ...
    trialFR, TrialSummary, ...
    Enabled=options.ExcludeFROutliers, ...
    Threshold=options.FROutlierThreshold, ...
    MinimumTrials=options.FROutlierMinimumTrials);
TrialSummary.HasFROutlier = ...
    frOutlierAudit.TrialOutlierObservationCount > 0;
TrialSummary.FROutlierObservationCount = ...
    frOutlierAudit.TrialOutlierObservationCount;

noStimMask = included & ~isElectricalStim;
stimMask = included & isElectricalStim;
pooledMask = included;

noStimWithZero = buildTuningGroup(trialFR, frOutlierMask, ...
    noStimMask, condition, ...
    coherenceIndex, direction, response, numel(conditionNames), ...
    numel(analysisCoherenceValues));
stimWithZero = buildTuningGroup(trialFR, frOutlierMask, ...
    stimMask, condition, ...
    coherenceIndex, direction, response, numel(conditionNames), ...
    numel(analysisCoherenceValues));
pooledWithZero = buildTuningGroup(trialFR, frOutlierMask, ...
    pooledMask, condition, ...
    coherenceIndex, direction, response, numel(conditionNames), ...
    numel(analysisCoherenceValues));
...
Neuro.FiringRateOutlierMask = frOutlierMask;
Neuro.FROutlierAudit = frOutlierAudit;
...
Neuro.Source = struct( ...
    ...
    'FROutlierSettings', frOutlierAudit.Settings);
...
fprintf(['Parsed %d tuning trials: %d NoStim and %d Stim; excluded %d. ' ...
    'FR outliers: %d observations in %d trials. ' ...
    'Quick-compatible nonzero output: %d NoStim and %d Stim.\n'], ...
    nnz(included), nnz(noStimMask), nnz(stimMask), nnz(~included), ...
    frOutlierAudit.OutlierObservationCount, ...
    frOutlierAudit.TrialWithAnyOutlierCount, ...
...
function tuning = buildTuningGroup(trialFR, frOutlierMask, ...
    groupMask, condition, ...
...
for index = groupIndices(:)'
    ...
    trialResponse = trialFR(index, :, :);
    trialOutliers = frOutlierMask(index, :, :);
    trialResponse(trialOutliers) = NaN;
    responses(row, column, replicate, :, :) = trialResponse;
end

if maxReplicates == 0
    means = nan(numConditions, numCoherences, numChannels, numUnits);
    sem = means;
    validCounts = zeros(size(means));
else
    ...
    responseStd = reshape(std(responses, 0, 3, 'omitnan'), ...
        [numConditions numCoherences numChannels numUnits]);
    validCounts = reshape(sum(isfinite(responses), 3), ...
        [numConditions numCoherences numChannels numUnits]);
    sem = responseStd ./ sqrt(validCounts);
    sem(validCounts == 0) = NaN;
end
...
tuning.Trials = struct('NumTrials', counts, 'Toward', toward, ...
    'Correct', correct, 'ValidNumTrials', validCounts, ...
    'OutlierCount', reshape(counts, ...
    [numConditions numCoherences 1 1]) - validCounts);
...
subset.Trials = struct( ...
    ...
    'ValidNumTrials', ...
    tuning.Trials.ValidNumTrials(:, coherenceMask, :, :), ...
    'OutlierCount', ...
    tuning.Trials.OutlierCount(:, coherenceMask, :, :));
...
summary.FROutlierFilter = frOutlierAudit;
