%% Compare Clay left-eye perspective-cue correct rates
% This script compares Clay's left-eye perspective cue (MonoL, condition 2)
...
behaviorFile = fullfile(repoRoot, 'BehaviorFitting', 'BehaviorData_Clay.mat');
outputDir = fullfile(repoRoot, 'BehaviorFitting', 'ClayLeftEyeCorrectRate');

monkeyName = "Clay";
conditionNames = ["Combined", "MonoL", "MonoR", "Bino"];
leftEyeConditionName = "MonoL";
...
[pairedDifferenceTable, pairedStatsTable] = compute_paired_condition_stats( ...
    sessionConditionTable, leftEyeConditionName, conditionNames);

mixedModelSummaryTable = table();
mixedModels = struct();
if runMixedEffectsModels
    [mixedModelSummaryTable, mixedModels] = fit_pairwise_mixed_models( ...
        conditionCoherenceTable, leftEyeConditionName, conditionNames);
end

%% Save outputs
sessionConditionPath = fullfile(outputDir, 'Clay_correct_rate_by_session_condition.csv');
conditionCoherencePath = fullfile(outputDir, 'Clay_correct_rate_by_session_condition_coherence.csv');
pairedDifferencePath = fullfile(outputDir, 'Clay_MonoL_minus_other_conditions_by_session.csv');
pairedStatsPath = fullfile(outputDir, 'Clay_MonoL_paired_stats.csv');
mixedModelSummaryPath = fullfile(outputDir, 'Clay_MonoL_pairwise_mixed_model_summary.csv');
resultMatPath = fullfile(outputDir, 'ClayLeftEyeCorrectRateResults.mat');
summaryPath = fullfile(outputDir, 'ClayLeftEyeCorrectRateSummary.txt');
...
save(resultMatPath, 'sessionConditionTable', 'conditionCoherenceTable', ...
    ...
    'mixedModels', 'behaviorFile', 'conditionNames', 'leftEyeConditionName', ...
    'blocksToAnalyze', '-v7.3');

write_summary_file(summaryPath, behaviorFile, outputDir, conditionNames, ...
    leftEyeConditionName, blocksToAnalyze, sessionConditionTable, ...
    pairedStatsTable, mixedModelSummaryTable);

if makeFigures
    make_correct_rate_figures(sessionConditionTable, conditionCoherenceTable, ...
        pairedDifferenceTable, conditionNames, leftEyeConditionName, outputDir);
end

disp(' ')
disp('Clay left-eye correct-rate comparison complete.')
...
assignin('base', 'clay_left_eye_session_condition_table', sessionConditionTable);
assignin('base', 'clay_left_eye_condition_coherence_table', conditionCoherenceTable);
assignin('base', 'clay_left_eye_paired_difference_table', pairedDifferenceTable);
assignin('base', 'clay_left_eye_paired_stats_table', pairedStatsTable);
assignin('base', 'clay_left_eye_mixed_model_summary_table', mixedModelSummaryTable);
assignin('base', 'clay_left_eye_mixed_models', mixedModels);
...
function [pairedDiffTable, statsTable] = compute_paired_condition_stats( ...
    sessionConditionTable, leftConditionName, conditionNames)

pairedDiffTable = table();
statsTable = table();
blocks = unique(sessionConditionTable.Block, 'stable');
comparators = conditionNames(conditionNames ~= leftConditionName);
...
function statsRow = summarize_paired_difference(blockRows, leftConditionName, ...
    comparatorName, blockName, differences)

leftRows = blockRows(blockRows.ConditionName == leftConditionName, :);
compRows = blockRows(blockRows.ConditionName == comparatorName, :);

pooledLeftCorrect = sum(leftRows.CorrectCount, 'omitnan');
pooledLeftTrials = sum(leftRows.TotalTrials, 'omitnan');
pooledCompCorrect = sum(compRows.CorrectCount, 'omitnan');
pooledCompTrials = sum(compRows.TotalTrials, 'omitnan');

pooledLeftRate = pooledLeftCorrect ./ pooledLeftTrials;
...
statsRow = table( ...
    string(blockName), string(leftConditionName + " - " + comparatorName), ...
    string(leftConditionName), string(comparatorName), nPairs, ...
    pooledLeftCorrect, pooledLeftTrials, pooledLeftRate, ...
    pooledCompCorrect, pooledCompTrials, pooledCompRate, ...
    meanDiff, medianDiff, semDiff, ciLow, ciHigh, effectDz, ...
    tP, tStat, tDf, signrankP, ...
    'VariableNames', {'Block', 'Comparison', 'LeftEyeCondition', 'ComparatorCondition', ...
    'NPairs', 'PooledLeftCorrect', 'PooledLeftTrials', 'PooledLeftRate', ...
...
function [summaryTable, models] = fit_pairwise_mixed_models( ...
    conditionCoherenceTable, leftConditionName, conditionNames)
...
for blockIdx = 1:numel(blocks)
    ...
    for compIdx = 1:numel(comparators)
        ...
        keepRows = conditionCoherenceTable.Block == blockName & ...
            (conditionCoherenceTable.ConditionName == leftConditionName | ...
        ...
        pairTable.IsLeftEye = categorical(pairTable.ConditionName == leftConditionName, ...
            [false true], ["Comparator", "LeftEye"]);
        ...
        modelName = matlab.lang.makeValidName(sprintf('%s_%s_vs_%s', ...
            blockName, leftConditionName, comparatorName));
        ...
        try
            model = fitglme(trialTable, ...
                'CorrectTrial ~ IsLeftEye * AbsCoherenceCentered + (1|SessionID)', ...
                'Distribution', 'Binomial');
            models.(modelName) = model;
            row = summarize_mixed_model(model, blockName, leftConditionName, comparatorName, ...
...
for rowIdx = 1:height(pairTable)
    ...
    IsLeftEye(rowRange) = string(pairTable.IsLeftEye(rowIdx));
...
trialTable = table( ...
    ...
    categorical(IsLeftEye(keepRows), ["Comparator", "LeftEye"]), ...
    ...
    'IsLeftEye', 'SignedCoherence', 'AbsCoherence', 'AbsCoherenceCentered'});
end

function row = summarize_mixed_model(model, blockName, leftConditionName, comparatorName, nRows, nTrials)
coefTable = model.Coefficients;
rowNames = string(coefTable.Name);

leftCoefIdx = find(contains(rowNames, "IsLeftEye_LeftEye"), 1);
interactionIdx = find(contains(rowNames, "IsLeftEye_LeftEye:AbsCoherenceCentered") | ...
    contains(rowNames, "AbsCoherenceCentered:IsLeftEye_LeftEye"), 1);

[leftEstimate, leftSE, leftP] = get_coef_values(coefTable, leftCoefIdx);
...
row = table(string(blockName), string(leftConditionName + " vs " + comparatorName), ...
    string(leftConditionName), string(comparatorName), nRows, ...
    nTrials, leftEstimate, leftSE, leftP, interactionEstimate, interactionSE, interactionP, ...
    ordinaryR2, ...
    'VariableNames', {'Block', 'Comparison', 'LeftEyeCondition', 'ComparatorCondition', ...
    'NRows', 'NExpandedTrials', 'LeftEyeLogOddsEstimate', 'LeftEyeLogOddsSE', 'LeftEyeLogOddsP', ...
    'LeftEyeByAbsCoherenceEstimate', 'LeftEyeByAbsCoherenceSE', ...
    'LeftEyeByAbsCoherenceP', 'OrdinaryR2'});
end
...
function make_correct_rate_figures(sessionConditionTable, conditionCoherenceTable, ...
    pairedDifferenceTable, conditionNames, leftConditionName, outputDir)
...
for blockIdx = 1:numel(blocks)
    ...
    fig = figure('Color', 'w', 'Name', sprintf('Clay_%s_MonoL_correct_rate', blockName));
    ...
    title(sprintf('%s MonoL paired differences', blockName), 'Interpreter', 'none');

    saveas(fig, fullfile(outputDir, sprintf('Clay_%s_MonoL_correct_rate_summary.fig', blockName)));
    exportgraphics(fig, fullfile(outputDir, sprintf('Clay_%s_MonoL_correct_rate_summary.png', blockName)), ...
        'Resolution', 300);

    fig2 = figure('Color', 'w', 'Name', sprintf('Clay_%s_correct_rate_by_coherence', blockName));
    ...
    saveas(fig2, fullfile(outputDir, sprintf('Clay_%s_correct_rate_by_abs_coherence.fig', blockName)));
    exportgraphics(fig2, fullfile(outputDir, sprintf('Clay_%s_correct_rate_by_abs_coherence.png', blockName)), ...
        'Resolution', 300);
end
end
...
yline(0, 'k--');
ylabel('MonoL correct rate - comparator')
...
fprintf(fid, 'Clay left-eye correct-rate comparison\n');
...
fprintf(fid, 'Left-eye condition: %s\n', leftEyeConditionName);
...
if ~isempty(mixedModelSummaryTable)
    ...
    for rowIdx = 1:height(mixedModelSummaryTable)
        ...
        fprintf(fid, '  %s %s: left-eye log-odds estimate=%.5f, p=%.5g; interaction p=%.5g\n', ...
            row.Block, row.Comparison, row.LeftEyeLogOddsEstimate, ...
            row.LeftEyeLogOddsP, row.LeftEyeByAbsCoherenceP);
    end
end
