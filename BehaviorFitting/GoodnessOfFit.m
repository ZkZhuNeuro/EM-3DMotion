...
%%
LL_N = NaN(size(unit_table, 1), 4);
LL_S = NaN(size(unit_table, 1), 4);
deviance_N = NaN(size(unit_table, 1), 4);
deviance_S = NaN(size(unit_table, 1), 4);
for i_rec = 1:size(unit_table, 1)
    disp([num2str(i_rec), '/', num2str(size(unit_table, 1))])
    for i_cue = 1:4
% for i_rec = 1
%     for i_cue = 1

        result_N = unit_table.Behave_N{i_rec}{i_cue};
        LL_N(i_rec, i_cue) = logLikelihood(result_N.data, result_N.options, ...
            result_N.Fit(1), result_N.Fit(2), result_N.Fit(3), result_N.Fit(4), result_N.Fit(5));

        result_S = unit_table.Behave_S{i_rec}{i_cue};
        LL_S(i_rec, i_cue) = logLikelihood(result_S.data, result_S.options, ...
            result_S.Fit(1), result_S.Fit(2), result_S.Fit(3), result_S.Fit(4), result_S.Fit(5));

        deviance_N(i_rec, i_cue) = result_N.deviance;
        deviance_S(i_rec, i_cue) = result_S.deviance;
    end
end
...
for i_cue = 1:4
    subplot(2, 2, i_cue)
hold on
histogram(LL_N(:, i_cue), 'BinWidth', 5)
histogram(LL_S(:, i_cue), 'BinWidth', 5)
...
for i_cue = 1:4
    subplot(2, 2, i_cue)
hold on
histogram(deviance_N(:, i_cue), 'BinWidth', 5)
histogram(deviance_S(:, i_cue), 'BinWidth', 5)
end
