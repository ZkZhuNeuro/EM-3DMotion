...
%  Settings
%  =========================
fieldName = 'Behav_mdl_full_deltaPSEpen_cap2';

K = 5;
lambdaDelta_grid = logspace(-4, 4, 20);   % strength of deltaPSE penalty
deltaMax = 0;                              % tolerated abs(deltaPSE) before penalty starts
...
for i_rec = 1:size(unit_table, 1)
    ...
    for i_cue = 1:size(Data_N{i_rec}, 2)
        ...
        [out, cv] = fit_full_deltaPSEPenalty_CV(X_full, y, n, ...
            lambdaDelta_grid, K, epsSlope, deltaMax, deltaCap);
...
for i_rec = 1:size(unit_table, 1)
    ...
    for i_cue = 1:4
        ...
        exitflag_all(i_rec, i_cue)= unit_table.(fieldName){i_rec}{i_cue}.exitflag;
    end
    unit_table.Bias_reg{i_rec} = Bias_fit(i_rec, :);
end
...
function [out, cv] = fit_full_deltaPSEPenalty_CV(X, y, n, lambdaDelta_grid, K, epsSlope, deltaMax, deltaCap)
% Cross-validated fitting for:
% J(b) = -logL + lambdaDelta * max(|deltaPSE|-deltaMax, 0)^2
% with hard constraint: abs(deltaPSE) <= deltaCap
    ...
    for iLam = 1:nLam
        ...
        for k = 1:K
            ...
            b_tr = fit_full_deltaPSEPenalty(X(tr,:), y(tr), n(tr), ...
                lam, epsSlope, deltaMax, deltaCap);
    ...
    b_hat = fit_full_deltaPSEPenalty(X, y, n, lamStar, epsSlope, deltaMax, deltaCap);
    ...
    out.exitflag = NaN;   % overwritten below only if desired
...
function b_hat = fit_full_deltaPSEPenalty(X, y, n, lambdaDelta, epsSlope, deltaMax, deltaCap)
% Fit full model with direct deltaPSE regularization + hard cap.

    if exist('fmincon', 'file') ~= 2
        error('This version requires fmincon because abs(deltaPSE) <= deltaCap is a nonlinear constraint.');
    end
    ...
    [b_hat, ~, exitflag] = fmincon(fun, b0, A, bineq, [], [], [], [], nonlcon, opts);

    % optional warning
    if exitflag <= 0
        % leave silent for batch runs
    end
end
...
% Heuristic to improve starting point for hard deltaPSE cap.

    b = make_feasible(b, epsSlope);
    ...
    for i = 1:numel(scales)
        ...
        b_try = make_feasible(b_try, epsSlope);
