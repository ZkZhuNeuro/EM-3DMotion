...
%  Settings
%  =========================
fieldName = 'Behav_mdl_full_lapse_deltaPSEpen';
...
for i_rec = 1:size(unit_table, 1)
    ...
    for i_cue = 1:size(Data_N{i_rec}, 2)
        ...
        [out, cv] = fit_full_withLapse_deltaPSEPenalty_CV(X_full, y, n, ...
            lambdaDelta_grid, K, epsSlope, deltaMax, lapseMax);
...
for i_rec = 1:size(unit_table, 1)
    ...
    for i_cue = 1:4
        ...
        lambdaD_all(i_rec, i_cue) = out.lambdaDelta;
...
function [out, cv] = fit_full_withLapse_deltaPSEPenalty_CV(X, y, n, lambdaDelta_grid, K, epsSlope, deltaMax, lapseMax)
% Cross-validated fitting for:
% p = gamma + (1-gamma-lambda)*sigmoid(eta)
% eta = b0 + b1*coh + b2*s + b3*(coh*s)
%
% Objective:
% J(theta) = -logL + lambdaDelta * max(|deltaPSE|-deltaMax, 0)^2
%
% theta = [b0 b1 b2 b3 gamma lambda]
    ...
    for iLam = 1:nLam
        ...
        for k = 1:K
            ...
            [theta_tr, ~] = fit_full_withLapse_deltaPSEPenalty(X(tr,:), y(tr), n(tr), ...
                lam, epsSlope, deltaMax, lapseMax);

            dev(iLam, k) = binom_deviance_withLapse(X(te,:), y(te), n(te), theta_tr);
        end
    end
    ...
    [theta_hat, exitflag] = fit_full_withLapse_deltaPSEPenalty(X, y, n, ...
        lamStar, epsSlope, deltaMax, lapseMax);
...
function [theta_hat, exitflag] = fit_full_withLapse_deltaPSEPenalty(X, y, n, lambdaDelta, epsSlope, deltaMax, lapseMax)
% Fit full model with lapse + direct deltaPSE regularization.
%
% theta = [b0 b1 b2 b3 gamma lambda]
    ...
    % initial guess for GLM coefficients
    b0 = [0; 1; 0; 0];
    try
        coh = X(:,1);
        s   = X(:,2);
    ...
    % Start with small lapse
    theta0 = [b0(:); 0.02; 0.02];

    fun = @(theta) nll_plus_deltaPSEPenalty_withLapse(X, y, n, theta, lambdaDelta, epsSlope, deltaMax);
    ...
    [theta_hat, ~, exitflag] = fmincon(fun, theta0, A, bineq, [], [], lb, ub, [], opts);
end

function J = nll_plus_deltaPSEPenalty_withLapse(X, y, n, theta, lambdaDelta, epsSlope, deltaMax)
% Penalized objective with lapse:
% J = NLL + lambdaDelta * max(|deltaPSE|-deltaMax, 0)^2

    nll = binom_nll_withLapse(X, y, n, theta);

    b = theta(1:4);
    deltaPSE = compute_deltaPSE(b);

    % Thresholded quadratic penalty on deltaPSE
    excess = max(abs(deltaPSE) - deltaMax, 0);
    pen_delta = lambdaDelta * excess^2;

    % Soft barrier for near-zero slopes
    b1 = theta(2);
    stimSlope = theta(2) + theta(4);

    pen_soft = 0;
    if b1 <= epsSlope
        pen_soft = pen_soft + 1e6 * (epsSlope - b1)^2;
    end
...
function D = binom_deviance_withLapse(X, y, n, theta)
% Held-out deviance using the lapse model
    b0 = theta(1);
    b1 = theta(2);
    b2 = theta(3);
    b3 = theta(4);
