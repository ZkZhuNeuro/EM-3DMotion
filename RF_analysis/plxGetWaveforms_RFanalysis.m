...
%     expression_SelIndex = [expression, 'SelIndex'];
    plx_file = recording_dir(~cellfun(@isempty, regexp({recording_dir.name}, strjoin(expression,'.*')))).name;
...
%     [~,name,~] = fileparts(FName{ifile});
    PName = cell2mat(unit_table{ifolder, 1});
    FName = cell2mat(unit_table{ifolder, 2});
