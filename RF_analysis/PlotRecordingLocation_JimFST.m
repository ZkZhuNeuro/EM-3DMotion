function [h, ML_Index, Depth_Voxel] = PlotRecordingLocation_JimFST(hole, guideTube, depth, offset)
...
% hole = [22,25];
% guideTube = 24.5;
[h, ML_Index, Depth_Voxel] = PlotRecordingLocationMRI_JimFST(OrigPoint_Voxel, ...
    Img_nii, ROI_nii_file, hole, guideTube + depth + offset(:, 3), ...
    ones(size(hole, 1), 1), ROI_intensity, color_mat, intensity_labels);

disp(guideTube + depth + offset(:, 3))
end