...
if strlength(outputDirectory) == 0
    outputDirectory = ...
        "C:\EM\PatternTuning\Archive\Previous_TDI_ADI_Results\Lo";
end
...
bodiCsvPath = fullfile(outputDirectory, 'FST3DBinocularOpticFlowDiscrimination.csv');
bodiSummaryCsvPath = fullfile(outputDirectory, 'FST3DBinocularOpticFlowSummary.csv');
bodiZeroTestsCsvPath = fullfile(outputDirectory, 'FST3DBinocularOpticFlowZeroTests.csv');
bodiFigurePath = fullfile(outputDirectory, 'FST3DBinocularOpticFlowDiscrimination.png');

save(matPath, 'PatternTuningTable', 'FastPatternTuningTable', ...
    'PatternTuningBySpeedTable', ...
    'PatternTuningSummary', 'ZeroTestResults', 'BinocularOpticFlowTable', ...
    'BinocularOpticFlowSummary', 'BODIZeroTestResults', ...
    'AnalysisConfig', '-v7.3');
...
MakeFSTBODIFigure(BinocularOpticFlowTable, BODIZeroTestResults, ...
    bodiFigurePath, ...
    "Lo 3D FST binocular optic-flow discrimination");
...
fprintf('Primary BODI mean/median: %.4f / %.4f\n', ...
...
function axesHandles = plot_speed_row(layout, T, zeroTests, speedCode, ...
