function [PatternTuningTable, PatternTuningBySpeedTable, PatternTuningSummary, ...
        ZeroTestResults, BinocularOpticFlowTable, ...
        BinocularOpticFlowSummary, BODIZeroTestResults] = ...
    RunEMFSTPatternTuningAnalysis(outputDirectory, inputPath)
%RUNEMFSTPATTERNTUNINGANALYSIS Run the TDI/ADI analysis on EM unit_table_gof.

arguments
    outputDirectory (1, 1) string = ""
    inputPath (1, 1) string = "C:\EM\PopulationAnalysis\unit_table_gof.mat"
end

if strlength(outputDirectory) == 0
    outputDirectory = ...
        "C:\EM\PatternTuning\Archive\Previous_TDI_ADI_Results\EM";
end
assert(isfile(inputPath), 'Input not found: %s', inputPath);

fprintf('Loading EM unit_table_gof from %s\n', inputPath);
loaded = load(inputPath, 'unit_table_gof');
assert(isfield(loaded, 'unit_table_gof') && istable(loaded.unit_table_gof), ...
    'The input MAT file does not contain table unit_table_gof.');

[PatternTuningTable, PatternTuningBySpeedTable, PatternTuningSummary, ...
    BinocularOpticFlowTable, BinocularOpticFlowSummary] = ...
    ComputeEMFSTPatternTuningIndices(loaded.unit_table_gof);
FastPatternTuningTable = PatternTuningBySpeedTable( ...
    PatternTuningBySpeedTable.SpeedRank_2D == 2, :);

ZeroTestResults = [ ...
    build_zero_test_results(PatternTuningTable, 1, "Slow (~4.2 deg/s)"); ...
    build_zero_test_results(FastPatternTuningTable, 2, "Fast (~12.5 deg/s)")];
...
BODIZeroTestResults = BuildFSTBODIZeroTestResults(BinocularOpticFlowTable);
...
AnalysisConfig.InputPath = inputPath;
AnalysisConfig.Selection = 'ROI==FST & ND==3D';
AnalysisConfig.EM3DDefinition = ...
    'ND==3D: p_AI for both perspective cues < 0.05 and Z3D_v_Z2D > 0';
AnalysisConfig.AnalyzedChannel = 'StimElec for each unit_table_gof row';
AnalysisConfig.CueOrder = ...
    '1=Combined, 2=left-eye perspective, 3=right-eye perspective, 4=Stereo';
AnalysisConfig.CoherenceEndpoints = ...
    'first stored tuning_mean column=-1 (away); last column=+1 (toward)';
AnalysisConfig.LateralAxes = ...
    'Raw2D_StimCh: direction x speed x eye x repeat; index 1=0 deg, index 5=180 deg';
AnalysisConfig.SpeedDegreesPerSecond = [4.166667, 4.166667 * 3];
AnalysisConfig.ResponseWindow = 'full stimulus interval; no baseline subtraction';
AnalysisConfig.PreferenceDefinition = ...
    'AI(Combined, StimElec) > 0: Toward; < 0: Away; == 0: Neutral';
AnalysisConfig.BODI = ...
...
AnalysisConfig.ZeroTest = ...
    'TDI/ADI: two-sided one-sample Wilcoxon signed-rank test against zero';
AnalysisConfig.BODIZeroTest = ...
    ['BODI: two-sided Wilcoxon rank-sum test against an equal-sized ' ...
    'zero reference sample'];
...
matPath = fullfile(outputDirectory, 'EMFST3DPatternTuningIndices.mat');
slowCsvPath = fullfile(outputDirectory, 'EMFST3DPatternTuningIndices_Slow.csv');
fastCsvPath = fullfile(outputDirectory, 'EMFST3DPatternTuningIndices_Fast.csv');
bySpeedCsvPath = fullfile(outputDirectory, 'EMFST3DPatternTuningIndices_BySpeed.csv');
summaryCsvPath = fullfile(outputDirectory, 'EMFST3DPatternTuningSummary.csv');
zeroTestsCsvPath = fullfile(outputDirectory, 'EMFST3DPatternTuningZeroTests.csv');
figurePath = fullfile(outputDirectory, 'EMFST3DPatternTuningIndices.png');
bodiCsvPath = fullfile(outputDirectory, 'EMFST3DBinocularOpticFlowDiscrimination.csv');
bodiSummaryCsvPath = fullfile(outputDirectory, 'EMFST3DBinocularOpticFlowSummary.csv');
bodiZeroTestsCsvPath = fullfile(outputDirectory, 'EMFST3DBinocularOpticFlowZeroTests.csv');
bodiFigurePath = fullfile(outputDirectory, 'EMFST3DBinocularOpticFlowDiscrimination.png');

save(matPath, 'PatternTuningTable', 'FastPatternTuningTable', ...
    'PatternTuningBySpeedTable', 'PatternTuningSummary', ...
    'ZeroTestResults', 'BinocularOpticFlowTable', ...
    'BinocularOpticFlowSummary', 'BODIZeroTestResults', ...
    'AnalysisConfig', '-v7.3');
writetable(PatternTuningTable, slowCsvPath);
...
writetable(BinocularOpticFlowTable, bodiCsvPath);
writetable(BinocularOpticFlowSummary, bodiSummaryCsvPath);
...
MakeFSTBODIFigure(BinocularOpticFlowTable, BODIZeroTestResults, ...
    bodiFigurePath, ...
    "EM 3D FST binocular optic-flow discrimination");

fprintf('\nEM 3D FST units (ROI==FST & ND==3D): %d\n', height(PatternTuningTable));
...
fprintf('Slow TDI mean/median: %.4f / %.4f\n', ...
    mean(PatternTuningTable.TDI, 'omitnan'), median(PatternTuningTable.TDI, 'omitnan'));
fprintf('Slow ADI mean/median: %.4f / %.4f\n', ...
    mean(PatternTuningTable.ADI, 'omitnan'), median(PatternTuningTable.ADI, 'omitnan'));
fprintf('BODI mean/median: %.4f / %.4f\n', ...
    mean(BinocularOpticFlowTable.BODI, 'omitnan'), ...
    median(BinocularOpticFlowTable.BODI, 'omitnan'));
...
title(layout, sprintf(['EM unit_table_gof 3D FST units: ' ...
...
fastAxes = plot_speed_row(layout, fastT, zeroTests, 2, ...
    'Fast (~12.5 deg/s)', tdiEdges, adiEdges);
