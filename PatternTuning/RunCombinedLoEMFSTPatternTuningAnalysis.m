function [CombinedPatternTuningTable, CombinedPatternTuningBySpeedTable, ...
    ...
    RunCombinedLoEMFSTPatternTuningAnalysis(outputDirectory, inputPaths)
%RUNCOMBINEDLOEMFSTPATTERNTUNINGANALYSIS Merge relaxed-Lo and EM analyses.
...
if strlength(outputDirectory) == 0
    outputDirectory = fullfile(fileparts(mfilename('fullpath')), 'outputs_combined');
end
...
[~, loBySpeed] = ComputeFSTPatternTuningIndices( ...
    lo3D.NeuroRespUnitTable, lo2D.LateralMotionRawFRTable, ...
    loMetadata.MIDTable, SelectionMode="positive-score");
[~, emBySpeed] = ComputeEMFSTPatternTuningIndices(emData.unit_table_gof);
...
AnalysisConfig.LoSelection = ...
    'ROI==FST & sig_Anova_CLR & Z3D_v_Z2D>0 (relaxed from Lo 1.28 criterion)';
AnalysisConfig.EMSelection = 'ROI==FST & ND==3D';
...
AnalysisConfig.ZeroTest = ...
    'Two-sided one-sample Wilcoxon signed-rank test against zero on pooled data';
...
matPath = fullfile(outputDirectory, 'CombinedLoEMFST3DPatternTuningIndices.mat');
slowCsvPath = fullfile(outputDirectory, 'CombinedLoEMFST3DPatternTuningIndices_Slow.csv');
fastCsvPath = fullfile(outputDirectory, 'CombinedLoEMFST3DPatternTuningIndices_Fast.csv');
bySpeedCsvPath = fullfile(outputDirectory, 'CombinedLoEMFST3DPatternTuningIndices_BySpeed.csv');
summaryCsvPath = fullfile(outputDirectory, 'CombinedLoEMFST3DPatternTuningSummary.csv');
zeroTestsCsvPath = fullfile(outputDirectory, 'CombinedLoEMFST3DPatternTuningZeroTests.csv');
figurePath = fullfile(outputDirectory, 'CombinedLoEMFST3DPatternTuningIndices.png');
...
make_summary_figure(CombinedPatternTuningTable, ...
    FastCombinedPatternTuningTable, ZeroTestResults, figurePath);
...
title(layout, sprintf(['Combined Lo (Z3D-Z2D>0) and EM 3D FST units: ' ...
