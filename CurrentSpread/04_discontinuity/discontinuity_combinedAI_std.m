...
for i_rec = 1:size(AI_comb_aligned, 1)
    ...
    %%
    step = 3;
    y4 = squeeze(AI_aligned(i_rec, :, 16-step:16+step));
    % figure(); hold on
    for i_cue = 1:4
    y = y4(i_cue, :);
    std_cues(i_rec, i_cue) = std(y, 'omitmissing');
    % y = AI_comb_aligned(i_rec, 16-step_sin:16+step_sin)';   % 7-element array
    t = (-step:step)'; % time points: 0,1,2,...6 (7 samples)
