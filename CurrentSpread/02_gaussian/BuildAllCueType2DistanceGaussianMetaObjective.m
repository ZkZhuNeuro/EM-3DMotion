...
for sigmaIndex = 1:numSigmas
    for condition = 1:4
        ...
        slope = double( ...
            cache.Statistics.WeightedSlope(condition, sigmaIndex));
        intercept = double( ...
            cache.Statistics.WeightedIntercept(condition, sigmaIndex));
...
for condition = 1:4
    ...
    objectiveTable.("Type2Slope_" + token) = ...
        cache.Statistics.WeightedSlope(condition, :)';
    objectiveTable.("Type2Intercept_" + token) = ...
        cache.Statistics.WeightedIntercept(condition, :)';
end
...
title('MT 2D Gaussian-meta optimization: distance to four Type II lines');
subtitle(sprintf('Best sigma %.6g; summed cue-wise mean squared distance %.6g; meta 2D N = %d', ...
