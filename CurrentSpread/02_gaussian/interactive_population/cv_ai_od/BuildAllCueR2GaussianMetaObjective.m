...
%
% The objective is the unweighted sum of four per-cue repeated
...
if strlength(options.OutputFolder) == 0
    options.OutputFolder = fullfile(cache.OutputFolder, options.UnitType, ...
        'AllCueR2Optimization');
end
sigmaValues = double(cache.SigmaValues(:));
ordinaryPerCueR2 = ...
    double(statistics.R2_AIxODWithinCondition);
...
[crossValidatedR2, crossValidatedSEM, r2ByRepeat] = ...
    calculateCrossValidatedCueR2(cache, pointValid, options.NumRepeats, ...
...
title(sprintf([ ...
    '%s %s Gaussian-meta optimization (%s OD): four-cue CV R^2 sum, ' ...
    'DeltaBias ~ AI + AI:OD'], ...
    result.Area, result.UnitType, result.ODDefinition));
subtitle(sprintf('Best sigma %.6g; summed CV R^2 %.4f; meta %s N = %d', ...
    bestSigma, bestValue, result.UnitType, ...
...
function [meanR2, semR2, r2ByRepeat] = ...
    calculateCrossValidatedCueR2( ...
    cache, pointValid, numRepeats, requestedFolds, seed)
