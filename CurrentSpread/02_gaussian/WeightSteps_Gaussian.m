...
%%
[sigma_hat, w_hat] = optimize_AI_weights(data_ori, behav, best_lambda);

[~, ~, N, M] = size(data_ori);   % N=31, M=56

% 1) build weights from sigma
% w = gaussian_weights(sigma_hat, N);   % 31x1
% w = [0 0 0 0 1 0 0 0 0];

weighted = sum( data_ori .* reshape(w_hat, 1,1,[],1), 3, 'omitnan');  % 8 x 20 x M
...
% --- initial guess ---
w0 = 1;          % start from uniform weights
...
w_hat = fmincon(@(sigma) objfun_AI_subset(sigma, data, behav, lambda_w, train_idx), ...
