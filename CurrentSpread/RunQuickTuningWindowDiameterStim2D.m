...
% from -4 through +4 contacts is required. MT and FST diameter distributions
% are overlaid with shared histogram bins.
...
audit = ComputeQuickTuningWindowDiameter(unitTable, ...
    ...
    ChannelMap=options.ChannelMap, RelativePositions=-4:4, ...
...
spanTable = summarizeDiameterSpans(audit, successful);
...
figureHandle = plotOverlaidHistograms( ...
    audit(successful, :), options.FigureVisible);
...
analysis.RelativePositions = -4:4;
...
analysis.CohortDefinition = [ ...
    ...
    "Complete live physical contacts -4 through +4"];
analysis.DiameterDefinition = [ ...
    "All 36 unique pair distances across nine contacts"; ...
...
save(fullfile(options.OutputFolder, ...
    'QuickTuningWindowDiameter_Stim2D_Minus4Plus4.mat'), ...
    'QuickTuningWindowDiameterStim2D', '-v7.3');

disp(summaryTable)
disp(spanTable)
fprintf('Nine-contact tuning-window diameter saved to %s\n', ...
    options.OutputFolder);
end


function figureHandle = plotOverlaidHistograms(audit, visible)
if visible
    visibility = 'on';
else
    visibility = 'off';
...
xlabel(axesHandle, ...
    'Nine-contact window diameter: CV squared tuning distance (z^2/condition)');
ylabel(axesHandle, 'Proportion of sessions');
title(axesHandle, sprintf([ ...
    'Stimulation-channel-defined 2D sessions, contacts -4 to +4\n' ...
    'MT N = %d; FST N = %d'], nnz(mt), nnz(fst)));
...
for index = 1:numel(groups)
    ...
    row = table(group, nnz(candidate), nnz(included), ...
        ...
        'CompleteNineContactSessions', 'ErrorSessions', 'MeanDiameterSquared', ...
...
function output = summarizeDiameterSpans(audit, successful)
areas = ["MT", "FST"];
output = table();
