...
%%
[Pred, AI_Dom_clst] = ClusterPrediction(squeeze(Z_aligned(1, :, :, :, :)), Z_aligned(2:3, :, :, :, :));
...
subplot(131)
hold on 
scatter(X_plot, [behav_Dom; behav_NonDom], 'r')
x = [X_plot, ones(size(X_plot, 1), 1)];
beta = x \ [behav_Dom; behav_NonDom];
slope = beta(1); 
intercept = beta(2); 
plot(x_plot, intercept + x_plot*slope,'-','Color','r','LineWidth',2.5);

X_ref = AI_ref_Dom .* abs(OD_ref);
X_ref = [X_ref; X_ref];
scatter(X_ref, [behav_Dom; behav_NonDom], 'b')
x = [X_ref, ones(size(X_ref, 1), 1)];
beta = x \ [behav_Dom; behav_NonDom];
slope = beta(1); 
intercept = beta(2); 
...
for i_sigma = 1:length(sigma)
    ...
    % 1) build weights from sigma
    w = gaussian_weights(sigma(i_sigma), 31);   % 31x1
    ...
    scatter(X, behav_pers(:, 1), 'filled', 'SizeData', 50, 'MarkerFaceColor', cmap(i_sigma, :), 'MarkerFaceAlpha', 0.05)
...
for i_sigma = 1:length(sigma)
    ...
    % w = [0 0 0 0 1 0 0 0 0];
    X = NaN(size(AI_Dom_clst, 1), 1);
    for i_rec = 1:size(AI_Dom_clst, 1)
        X_rec = 0;
        weight_sum = 0;
        for i_clst = 1:size(AI_Dom_clst{i_rec}, 2)
            weight_sum = weight_sum + w(AI_Dom_clst{i_rec}(2, i_clst) + 16);
        end

        for i_clst = 1:size(AI_Dom_clst{i_rec}, 2)
            weight = w(AI_Dom_clst{i_rec}(2, i_clst) + 16) / weight_sum;
            X_rec = X_rec + AI_Dom_clst{i_rec}(1, i_clst) * weight;
        end
        X(i_rec) = X_rec;
...
%%
function [Pred, AI_Dom] = ClusterPrediction(Z_train, Z_train_pers)
...
AI_Dom = cell(size(AI_comb_train, 1), 1);
for i_rec = 1:size(AI_comb_train, 1)
    Pred_rec = NaN(2, size(good_groups{i_rec}, 2));
    AI_rec = NaN(2, size(good_groups{i_rec}, 2));
    for i_group = 1: size(good_groups{i_rec}, 2)
        ...
        if OD > 0
            ...
            AI_rec(1, i_group) = AI_Left;
            AI_rec(2, i_group) = dist;
        else
            Pred_rec(1, i_group) = AI_Right * abs(OD);
            Pred_rec(2, i_group) = dist;
            AI_rec(1, i_group) = AI_Right;
            AI_rec(2, i_group) = dist;
        end
