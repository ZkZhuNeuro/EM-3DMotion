...
%%
[sigma_hat, w_hat] = optimize_AI_weights(Pred, behav_pers);
...
w = gaussian_weights(sigma_hat, N);   % 31x1
...
for i_rec = 1:M
    ...
    X(i_rec) = X_rec; 
    AI_clst(i_rec) = AI_rec; 
    OD_clst(i_rec) = OD_rec; 
...
%%
figure(); hold on
scatter(AI_clst, behav_pers(:, 1));
Old = AI_ref_Dom;
scatter(Old, behav_pers(:, 1));
for i_rec = 1:size(behav, 1)
    if xor(AI_clst(i_rec) > Old(i_rec), behav_pers(i_rec, 1) > 0)
    plot([AI_clst(i_rec), Old(i_rec)], [behav_pers(i_rec, 1), behav_pers(i_rec, 1)], 'Color', 'r')
    else
    plot([AI_clst(i_rec), Old(i_rec)], [behav_pers(i_rec, 1), behav_pers(i_rec, 1)], 'Color', 'b')
    end
end
axis square
xlim([-1 1])
