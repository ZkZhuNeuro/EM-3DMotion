...
% Y = Behav_slope_N - Behav_slope_S for the combined cue, where slopes
% are the coherence coefficients from the sigmoid/logistic GLM.
...
required_variables = { ...
    ...
    'Behav_slope_N', 'Behav_slope_S', 'Behav_slope_NminusS'};
...
slope_nonstim = stack_cue_values(unit_table_gof.Behav_slope_N, 4);
slope_stim = stack_cue_values(unit_table_gof.Behav_slope_S, 4);
slope_change = stack_cue_values(unit_table_gof.Behav_slope_NminusS, 4);

combined_cue = 1;
valid_combined_slope = goodfit_nonstim(:, combined_cue) & ...
    goodfit_stim(:, combined_cue) & ...
    isfinite(slope_nonstim(:, combined_cue)) & ...
    isfinite(slope_stim(:, combined_cue)) & ...
    isfinite(slope_change(:, combined_cue)) & ...
    abs(slope_change(:, combined_cue) - ...
    (slope_nonstim(:, combined_cue) - slope_stim(:, combined_cue))) < 1e-10;
combined_slope_change = slope_change(:, combined_cue);
combined_slope_change(~valid_combined_slope) = nan;
combined_slope_nonstim = slope_nonstim(:, combined_cue);
combined_slope_nonstim(~valid_combined_slope) = nan;
combined_slope_stim = slope_stim(:, combined_cue);
combined_slope_stim(~valid_combined_slope) = nan;
...
session_change_table = table( ...
    ...
    'MaxAbsBiasChange', 'CombinedSlopeNonStim', 'CombinedSlopeStim', ...
    'CombinedSlopeChange', 'NValidBiasCues', ...
...
ylabel(ax, 'Combined-cue sigmoid slope change, non-stim - stim');
