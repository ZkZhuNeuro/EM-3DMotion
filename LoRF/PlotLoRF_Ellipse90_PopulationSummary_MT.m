...
%% Population summary of saved 90% ellipse fits.
% This script reads the saved ellipse-fit table and generates the
% population-level cumulative overlap figure for the saved 90% ellipses.

inputMat = 'C:\Users\zzhu329\Documents\GitHub\EM-3DMotion\LoRF\LoRF_unit_table_jim_mt_with_ellipse.mat';
outputDir = 'C:\LoData\RF\OldVsNewFitComparison_MT';
figurePath = fullfile(outputDir, 'LoRF_Ellipse90_PopulationCumulative_MT.png');
regressionFigurePath = fullfile(outputDir, 'LoRF_Ellipse90_HoleY_vs_Eccentricity_MT.png');
logFile = fullfile(outputDir, 'PlotLoRF_Ellipse90_PopulationAnalysis_MT.log');
...
title(axNew, sprintf('Jim MT 90%% ellipse fits | reach x=%g deg: %d/%d (%.1f%%)', ...
    crossingLineDeg, newCrossCount, newCount, 100 * safeProportion(newCrossCount, newCount)), ...
    'Interpreter', 'none');

sgtitle(tileObj, 'Population cumulative 90% RF ellipses', 'Interpreter', 'none');
