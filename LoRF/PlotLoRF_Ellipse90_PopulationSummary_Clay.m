...
inputMat = 'C:\LoData\RF\LoRF_unit_table_clay_with_ellipse.mat';
outputDir = 'C:\LoData\RF\OldVsNewFitComparison_Clay';
figurePath = fullfile(outputDir, 'LoRF_Ellipse90_PopulationCumulative_Clay.png');
regressionFigurePath = fullfile(outputDir, 'LoRF_Ellipse90_HoleY_vs_Eccentricity_Clay.png');
logFile = fullfile(outputDir, 'PlotLoRF_Ellipse90_PopulationAnalysis_Clay.log');
...
title(axNew, sprintf('Clay 90%% ellipse fits | reach x<=%g deg: %d/%d (%.1f%%)', ...
    crossingLineDeg, newCrossCount, newCount, 100 * safeProportion(newCrossCount, newCount)), ...
    'Interpreter', 'none');

sgtitle(tileObj, 'Clay population cumulative 90% RF ellipses', 'Interpreter', 'none');
