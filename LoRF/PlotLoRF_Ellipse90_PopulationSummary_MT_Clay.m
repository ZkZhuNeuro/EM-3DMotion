...
%% Population summary of saved 90% ellipse fits for Clay.
% This script reads the saved Clay ellipse-fit table and generates the
% population-level cumulative overlap figure for the saved 90% ellipses.
% Clay recordings were in the left hemisphere, so the crossing boundary is
% defined as x <= -xResolutionDeg.

inputMat = 'C:\LoData\RF\LoRF_unit_table_clay_mt_with_ellipse.mat';
outputDir = 'C:\LoData\RF\OldVsNewFitComparison_MT_Clay';
figurePath = fullfile(outputDir, 'LoRF_Ellipse90_PopulationCumulative_MT_Clay.png');
regressionFigurePath = fullfile(outputDir, 'LoRF_Ellipse90_HoleY_vs_Eccentricity_MT_Clay.png');
logFile = fullfile(outputDir, 'PlotLoRF_Ellipse90_PopulationAnalysis_MT_Clay.log');
...
logMessage(logFile, 'Clay left-hemisphere boundary: x<=-xResolutionDeg = %g deg', crossingLineDeg);
logMessage(logFile, 'Ellipse90 reach x<=%g deg: %d/%d (%.2f%%)', ...
    crossingLineDeg, summary.ellipseCountCross, summary.ellipseCountTotal, 100 * summary.ellipsePropCross);
logMessage(logFile, 'Ellipse90 center x<=%g deg: %d/%d (%.2f%%)', ...
    crossingLineDeg, summary.centerCountCross, summary.centerCountTotal, 100 * summary.centerPropCross);
logMessage(logFile, 'Ellipse90 covers (0,0): %d/%d (%.2f%%)', ...
...
fprintf('Clay left-hemisphere boundary: x<=-xResolutionDeg = %g deg\n', crossingLineDeg);
fprintf('Ellipse90 reach x<=%g deg: %d/%d (%.2f%%)\n', ...
    crossingLineDeg, summary.ellipseCountCross, summary.ellipseCountTotal, 100 * summary.ellipsePropCross);
fprintf('Ellipse90 center x<=%g deg: %d/%d (%.2f%%)\n', ...
    crossingLineDeg, summary.centerCountCross, summary.centerCountTotal, 100 * summary.centerPropCross);
fprintf('Ellipse90 covers (0,0): %d/%d (%.2f%%)\n', ...
    summary.fixationCoverCount, summary.ellipseCountTotal, 100 * summary.fixationCoverProp);
...
for idx = unitIndices
    if hasEllipse90Fit(RF_table, idx)
        xDeg = tableCell(RF_table.EllipseX_deg, idx);
        yDeg = tableCell(RF_table.EllipseY_deg, idx);
        ...
        if ~isempty(xDeg)
            ...
            newCount = newCount + 1;
            newCrossCount = newCrossCount + double(outlineReachesVerticalLineLeft(xDeg, crossingLineDeg));
            fixationCoverCount = fixationCoverCount + double(outlineCoversPoint(xDeg, yDeg, 0, 0));

            if hasEllipse90Center(RF_table, idx)
                centerDeg = tableCell(RF_table.EllipseCenter_deg, idx);
...
title(axNew, sprintf('Clay MT 90%% ellipse fits | reach x<=%g deg: %d/%d (%.1f%%)', ...
    crossingLineDeg, newCrossCount, newCount, 100 * safeProportion(newCrossCount, newCount)), ...
    'Interpreter', 'none');

sgtitle(tileObj, 'Clay MT population cumulative 90% RF ellipses', 'Interpreter', 'none');
...
function tf = hasEllipse90Fit(RF_table, idx)
...
function tf = hasEllipse90Center(RF_table, idx)
...
function tf = outlineReachesVerticalLineLeft(xDeg, lineDeg)
xDeg = xDeg(:);
