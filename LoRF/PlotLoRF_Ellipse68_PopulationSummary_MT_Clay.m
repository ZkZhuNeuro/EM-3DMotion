...
%% Population summary of saved 68% ellipse fits for Clay.
% This script reads the saved Clay ellipse-fit table and generates only the
...
figurePath = fullfile(outputDir, 'LoRF_Ellipse68_PopulationCumulative_MT_Clay.png');
regressionFigurePath = fullfile(outputDir, 'LoRF_Ellipse68_HoleY_vs_Eccentricity_MT_Clay.png');
logFile = fullfile(outputDir, 'PlotLoRF_Ellipse68_PopulationAnalysis_MT_Clay.log');
...
title(axNew, sprintf('Clay MT 68%% ellipse fits | reach x<=%g deg: %d/%d (%.1f%%)', ...
    crossingLineDeg, newCrossCount, newCount, 100 * safeProportion(newCrossCount, newCount)), ...
    'Interpreter', 'none');

sgtitle(tileObj, 'Clay MT population cumulative 68% RF ellipses', 'Interpreter', 'none');
