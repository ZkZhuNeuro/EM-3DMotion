...
figurePath = fullfile(outputDir, 'LoRF_Ellipse68_PopulationCumulative_Clay.png');
regressionFigurePath = fullfile(outputDir, 'LoRF_Ellipse68_HoleY_vs_Eccentricity_Clay.png');
logFile = fullfile(outputDir, 'PlotLoRF_Ellipse68_PopulationAnalysis_Clay.log');
...
title(axNew, sprintf('Clay 68%% ellipse fits | reach x<=%g deg: %d/%d (%.1f%%)', ...
    crossingLineDeg, newCrossCount, newCount, 100 * safeProportion(newCrossCount, newCount)), ...
    'Interpreter', 'none');

sgtitle(tileObj, 'Clay population cumulative 68% RF ellipses', 'Interpreter', 'none');
