...
for recording_num = 1:length(inclusion_folder_indices)
    ...
    recording_dir = dir(fullfile(baseRecordingPath, '*.mat'));

    names = {recording_dir.name};
    lowerNames = lower(names);
    isMatch = contains(lowerNames, 'sparsenoise') & contains(lowerNames, 'sorted-');

    matches = recording_dir(isMatch);

    wfexp_file = reshape({matches.name}, [], 1);
    if isempty(wfexp_file)
        warning('No waveform export files matched in %s', baseRecordingPath);
    end
    ...
    pat = '(?i)tt(\d+).*sorted-(\d+).*sparsenoise(?:-(\d+))?';
