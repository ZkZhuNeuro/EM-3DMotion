function [rawRFmap, uniXPos, uniYPos, meanXYpos, RFmapTable_allSti, SpikeRate_Baseline] = RFMappingFunction_Lo_Jim(unit_table, i_tt, i_unit, unitProgressText, targetUnitID)
%% Settings
% ElectrodeNums = [1:16];
...
for unitLoop = 1
    ...
    tok = regexp(fn, '(?i)tt(\d+).*sorted-(\d+)', 'tokens', 'once');
    if isempty(tok)
        tok = {'NaN', 'NaN'};
    end
