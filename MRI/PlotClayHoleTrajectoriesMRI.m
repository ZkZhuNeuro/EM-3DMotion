function summary = PlotClayHoleTrajectoriesMRI(outputDir, guideDepth, maxDepth, xls_tables)
% Plot all unique Clay recording holes as vertical MRI trajectories grouped by coronal slice.
...
if nargin < 4 || isempty(xls_tables)
    xls_tables = {
...
if ischar(xls_tables) || isstring(xls_tables)
    xls_tables = cellstr(string(xls_tables));
end
...
holes = [];
for t = 1:numel(xls_tables)
    tb = loadRecordingTableLocal(xls_tables{t});
    holes = [holes; parseHoleColumn(tb.Hole)]; %#ok<AGROW>
end
...
if isempty(holes)
    error('No valid positive-X hole coordinates were found in the requested Clay tables.');
end
...
sourceTableLabel = strjoin(string(xls_tables), ' | ');

summary = table('Size', [numel(AP_holes), 5], ...
    'VariableTypes', {'double', 'double', 'string', 'string', 'string'}, ...
    'VariableNames', {'Y', 'HoleCount', 'XHoles', 'FigureFile', 'SourceTables'});
...
for s = 1:numel(AP_holes)
    ...
    title(sprintf('Clay MRI coronal slice Y = %d | %d trajectories | depth %.1f to %.1f', ...
        slice, size(sliceHoles, 1), guideDepth, maxDepth), 'FontSize', 12);
    hold off;

    outFile = fullfile(outputDir, sprintf('Clay_HoleTrajectories_Y%02d.png', slice));
    ...
    summary.SourceTables(s) = sourceTableLabel;
end

writetable(summary, fullfile(outputDir, 'Clay_HoleTrajectorySummary.csv'));
end

function tb = loadRecordingTableLocal(xls_table)
raw = readcell(xls_table);
header = matlab.lang.makeValidName(string(raw(1, :)));
holeColumnIndex = find(header == "Hole", 1);

if isempty(holeColumnIndex)
    error('Unable to locate a Hole column in %s.', xls_table);
end

holeData = raw(2:end, holeColumnIndex);
tb = table(string(holeData), 'VariableNames', {'Hole'});
end
...
for i = 1:numel(holeStrings)
    ...
    if ismissing(holeText) || strlength(holeText) == 0 || strcmpi(holeText, "missing")
        continue
    end
